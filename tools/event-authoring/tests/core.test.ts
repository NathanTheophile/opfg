...
describe('Content Schema v10', () => {
  it('uses current schema and accepts Normal, Scheduled, Critical, current NPC/location/trait contracts', () => {
    const project = createDemoProject();
...
describe('Event import/export', () => {
  ...
  it('round-trips an OPFG bundle with manifest, Events and localization dictionaries', async () => {
    ...
    expect(read.manifest).toMatchObject({ format: 'opfg-events-bundle', version: 1, schemaVersion: 10, eventCount: project.events.length });
...
describe('v0.3 project migration and validation', () => {
  it('migrates safe fields, removes obsolete concepts, and marks ambiguous changes for review', () => {
    ...
    expect(result.project.gameSchemaVersion).toBe(10);
  ...
  it('keeps flags authoring-only and exports the current ContentCatalog', () => {
    ...
    expect(result.catalog?.schemaVersion).toBe(10);
