...
import { NPC_STAT_IDS, PLAYER_STAT_IDS, type StatId } from '../gameSchema/current/contract';
...
type RegistryTab = 'races' | 'seas' | 'affiliations' | 'familyStructures' | 'socialClasses' | 'traits' | 'items' | 'ships' | 'crewRoles' | 'npcs' | 'locations' | 'flags';
const TABS: RegistryTab[] = ['races','seas','affiliations','familyStructures','socialClasses','traits','items','ships','crewRoles','npcs','locations','flags'];
...
export default function RegistryPanel({ open, project, activeLocale, onClose, onChange, onLocalizedTextChange }: Props) {
  ...
  const add = () => {
    ...
    if (tab === 'locations') next.registries.locations.push({ id, seaId: null, blocksScheduledEvents: false, allowsShipSale: false });
