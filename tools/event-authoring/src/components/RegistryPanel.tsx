...
import { DEVIL_FRUIT_TAGS, DEVIL_FRUIT_TYPES, LOCATION_SERVICES, LOCATION_TAGS, NPC_STAT_IDS, PLAYER_STAT_IDS, type StatId } from '../gameSchema/current/contract';
...
export default function RegistryPanel({ open, project, activeLocale, onClose, onChange, onLocalizedTextChange }: Props) {
  ...
  const add = () => {
    ...
    if (tab === 'devilFruits') { const nameKey = `devilFruit.${id}.name`; next.registries.devilFruits.push({ id, nameKey, type: 'paramecia', playableV1: false, itemId: null, tags: [] }); next.localization = ensureKeys(next.localization, [nameKey]); }
    ...
    if (tab === 'locations') { const nameKey = `location.${id}.name`; next.registries.locations.push({ id, nameKey, seaId: r.seas[0]?.id ?? '', type: 'island', parentLocationId: null, canBeBirthLocation: false, blocksScheduledEvents: false, allowsDocking: false, shipMarket: 'none', services: [], tags: [] }); next.localization = ensureKeys(next.localization, [nameKey]); }
  ...
  return <div className="modal-backdrop"><div className="registry-panel">
    ...
    <div className="registry-list">
      ...
      {tab === 'locations' && r.locations.map((entry, index) => <div className="registry-card" key={entry.id}><code>{entry.id}</code>{localized(entry.nameKey, 'Name')}<IdSelect value={entry.seaId} options={r.seas} onChange={(seaId) => { const next = structuredClone(project); next.registries.locations[index].seaId = seaId; onChange(next); }} /><label className="field"><span>Ship market</span><select value={entry.shipMarket} onChange={(e) => { const next = structuredClone(project); next.registries.locations[index].shipMarket = e.target.value as typeof entry.shipMarket; onChange(next); }}><option value="none">none</option><option value="small_craft">small_craft</option><option value="full">full</option></select></label><label className="field"><span>Services</span><input value={entry.services.join(', ')} onChange={(e) => { const next = structuredClone(project); next.registries.locations[index].services = e.target.value.split(',').map((x) => x.trim()).filter((x) => LOCATION_SERVICES.includes(x as never)) as typeof entry.services; onChange(next); }} /></label><label className="field"><span>Tags</span><input value={entry.tags.join(', ')} onChange={(e) => { const next = structuredClone(project); next.registries.locations[index].tags = e.target.value.split(',').map((x) => x.trim()).filter((x) => LOCATION_TAGS.includes(x as never)) as typeof entry.tags; onChange(next); }} /></label><label className="checkbox-inline"><input type="checkbox" checked={entry.blocksScheduledEvents} onChange={(e) => { const next = structuredClone(project); next.registries.locations[index].blocksScheduledEvents = e.target.checked; onChange(next); }} /> blocks scheduled</label><label className="checkbox-inline"><input type="checkbox" checked={entry.allowsDocking} onChange={(e) => { const next = structuredClone(project); next.registries.locations[index].allowsDocking = e.target.checked; onChange(next); }} /> docking</label><button className="danger-text" onClick={() => remove(entry.id)}>Delete</button></div>)}
      {tab === 'devilFruits' && r.devilFruits.map((entry, index) => <div className="registry-card" key={entry.id}><div className="registry-id"><code>{entry.id}</code><button className="danger-text" onClick={() => remove(entry.id)}>Delete</button></div>{localized(entry.nameKey, 'Name')}<label className="field"><span>Type</span><select value={entry.type} onChange={(e) => { const next = structuredClone(project); next.registries.devilFruits[index].type = e.target.value as typeof entry.type; onChange(next); }}>{DEVIL_FRUIT_TYPES.map((x) => <option key={x}>{x}</option>)}</select></label><label className="checkbox-inline"><input type="checkbox" checked={entry.playableV1} onChange={(e) => { const next = structuredClone(project); next.registries.devilFruits[index].playableV1 = e.target.checked; next.registries.devilFruits[index].itemId = e.target.checked ? '' : null; onChange(next); }} /> playable V1</label>{entry.playableV1 && <label className="field"><span>Physical Item</span><IdSelect value={entry.itemId ?? ''} options={r.items} onChange={(itemId) => { const next = structuredClone(project); next.registries.devilFruits[index].itemId = itemId; onChange(next); }} /></label>}<label className="field"><span>Tags</span><input value={entry.tags.join(', ')} onChange={(e) => { const next = structuredClone(project); next.registries.devilFruits[index].tags = e.target.value.split(',').map((x) => x.trim()).filter((x) => DEVIL_FRUIT_TAGS.includes(x as never)); onChange(next); }} /></label></div>)}
