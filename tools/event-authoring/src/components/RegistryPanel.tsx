...
import { affiliationNameKey, itemNameKey, npcNameKey, raceNameKey, seaNameKey, traitDescriptionKey, traitNameKey } from '../localization/keys';
import { ensureKeys } from '../localization/store';
import { IdSelect, NumberInput } from './EditorPrimitives';
import LocalizedField from './LocalizedField';

type RegistryTab = 'races' | 'seas' | 'affiliations' | 'traits' | 'items' | 'npcs' | 'locations' | 'flags';
const TABS: RegistryTab[] = ['races','seas','affiliations','traits','items','npcs','locations','flags'];
...
export default function RegistryPanel({ open, project, activeLocale, onClose, onChange, onLocalizedTextChange }: Props) {
  ...
  const add = () => {
    ...
    if (tab === 'npcs') { next.registries.npcs.push({ id, nameKey: npcNameKey(id), raceId: null, originSeaId: null, affiliationId: null, initialStats: createNpcStats() }); next.localization = ensureKeys(next.localization, [npcNameKey(id)]); }
    if (tab === 'locations') next.registries.locations.push({ id, blocksScheduledEvents: false });
