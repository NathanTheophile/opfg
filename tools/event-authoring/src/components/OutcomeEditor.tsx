...
export default function OutcomeEditor({ value, onChange, registries, eventIds, scheduledEventIds, localization, activeLocale, sourceLocale, onLocalizedTextChange }: Props) {
  return <div className="outcome-editor">
    ...
    <div className="stack">
      {value.effects.map((effect, index) => <EffectEditor key={index} value={effect} registries={registries} eventIds={eventIds} scheduledEventIds={scheduledEventIds} onChange={(next) => { const effects = [...value.effects]; effects[index] = next; onChange({ ...value, effects }); }} onRemove={() => onChange({ ...value, effects: value.effects.filter((_, i) => i !== index) })} />)}
