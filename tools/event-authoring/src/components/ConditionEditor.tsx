...
const TYPES: Condition['type'][] = [
  'all','any','not','hasTrait','statAtLeast','hasFlag','hasItem','locationIs','isAtSea','isOnLand','careerPhaseIs',
  'ageAtLeastMonths','ageAtMostMonths','shipConditionAtLeast','shipConditionAtMost','npcStatusIs','npcRelationshipAtLeast',
...
export default function ConditionEditor({ value, onChange, registries, eventIds }: Props) {
  ...
  const body = (() => {
    switch (value.type) {
      ...
      case 'isAtSea': case 'isOnLand': return <span className="muted">No parameters</span>;
      ...
      case 'shipConditionAtLeast': case 'shipConditionAtMost': return <NumberInput value={value.value} onChange={(next) => onChange({ ...value, value: next })} />;
