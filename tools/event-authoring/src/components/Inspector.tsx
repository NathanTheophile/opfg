...
export default function Inspector(props: Props) {
  const { event, contentFolder, notes, status, issues, registries, eventIds, scheduledEventIds, localization, activeLocale, sourceLocale, supportedLocales, onLocaleChange, onLocalizedTextChange, onChange, onContentFolderChange, onRename, onNotesChange, onStatusChange } = props;
  ...
  return <aside className="inspector"><div className="inspector-scroll">
    ...
    <Section title="General">
      ...
      <div className="two-columns">
        <Field label="Kind"><select value={event.kind} onChange={(e) => onChange(convertKind(event, e.target.value as EventDefinition['kind']))}><option value="normal">normal</option><option value="scheduled">scheduled</option><option value="critical">critical</option></select></Field>
        <Field label="Content folder"><select value={contentFolder} disabled={event.kind !== 'normal'} onChange={(e) => onContentFolderChange(e.target.value as ContentFolder)}>{event.kind === 'scheduled' ? <option value="scheduled">scheduled</option> : event.kind === 'critical' ? <option value="critical">critical</option> : normalFolders.map((folder) => <option key={folder} value={folder}>{folder}</option>)}</select></Field>
      </div>
    ...
    <Section title={`Choices · ${event.choices.length}`}><div className="stack">
      {event.choices.map((choice, index) => <ChoiceEditor key={`${choice.id}-${index}`} eventId={event.id} value={choice} registries={registries} eventIds={eventIds} scheduledEventIds={scheduledEventIds} localization={localization} activeLocale={activeLocale} sourceLocale={sourceLocale} onLocalizedTextChange={onLocalizedTextChange} onChange={(next) => { const choices = [...event.choices]; choices[index] = next; onChange({ ...event, choices }); }} onRemove={() => onChange({ ...event, choices: event.choices.filter((_, i) => i !== index) })} />)}
