...
import { eventKeys, outcomeKey, choiceKey, raceNameKey, seaNameKey, affiliationNameKey, traitNameKey, traitDescriptionKey, itemNameKey, npcNameKey, modifierLabelKey } from '../localization/keys';
...
export const createDemoProject = (): AuthoringProject => {
  ...
  const squall: EventDefinition = {
    id: 'black_squall_demo', ...eventKeys('black_squall_demo'), kind: 'normal', eligibility: { type: 'isAtSea' }, choices: [{ id: 'navigate', textKey: choiceKey('black_squall_demo','navigate'), resolution: { type: 'dice', statId: 'navigation', successThreshold: 13, modifiers: [{ condition: { type: 'shipConditionAtMost', value: 25 }, value: -2, displayLabelKey: modifierLabelKey('black_squall_demo','navigate',0) }], outcomes: {
      criticalFailure: { id: 'cf', textKey: outcomeKey('black_squall_demo','navigate','cf'), effects: [{ type: 'modifyShipCondition', amount: -10 }] },
      failure: { id: 'f', textKey: outcomeKey('black_squall_demo','navigate','f'), effects: [{ type: 'modifyShipCondition', amount: -5 }] },
  ...
  const registries: GameRegistries = {
    ...
    locations: [{ id: 'starter_port', blocksScheduledEvents: false }, { id: 'open_sea', blocksScheduledEvents: false }, { id: 'isolated_cove', blocksScheduledEvents: true }],
    ...
    items: [{ id: 'sealed_chart', nameKey: itemNameKey('sealed_chart') }], npcs: [{ id: 'mira', nameKey: npcNameKey('mira'), raceId: null, originSeaId: null, affiliationId: null, initialStats: { health: 25, morale: 25, strength: 25, observation: 25, intelligence: 25, luck: 25, loyalty: 25, calm: 25 } }],
  ...
  return { authoringVersion: 4, gameSchemaVersion: CONTENT_SCHEMA_VERSION, name: 'OPFG Events Demo', sourceLocale: 'fr', supportedLocales: ['fr','en'], events, nodes: [
