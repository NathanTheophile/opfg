...
describe('save v15', () => {
  ...
  it('round-trips every v11 field including Origins profile, unbounded health, leadership, passengers, and inventories', () => {
    ...
    state.npcs.guest = { raceId: null, status: 'known', relationship: 0, powers: createDefaultPowerState(), stats: { health: 25, morale: 25, strength: 25, observation: 25, intelligence: 25, luck: 25, loyalty: 25, calm: 25 } };
    ...
    state.npcs.mira = { raceId: null, status: 'dead', relationship: 4, powers: createDefaultPowerState(), stats: { health: 0, morale: 10, strength: 10, observation: 10, intelligence: 10, luck: 10, loyalty: 10, calm: 10 } };
  ...
  it('migrates v7 and rejects v6 and invalid non-active slots', () => {
    ...
    expect(migrated).toMatchObject({ version: 15, ship: { shipId: 'sloop', health: 20 }, isLeader: true, passengerNpcIds: [], berries: 0, player: { career: { affiliationId: 'civilian', reputation: 0, bounty: 0 } }, endingId: null });
    ...
    expect(deserializeGameState(JSON.stringify(v8))).toMatchObject({ version: 15, isLeader: true, passengerNpcIds: [] });
    ...
    expect(deserializeGameState(JSON.stringify(v9))).toMatchObject({ version: 15, player: { stats: { agility: 25 }, profile: { familyStructureId: null, socialClassId: null } } });
    ...
    expect(deserializeGameState(JSON.stringify(v10))).toMatchObject({ version: 15, player: { profile: { familyStructureId: null, socialClassId: null } } });
