...
describe('save v9', () => {
  it('round-trips every v9 field including leadership, passengers, stacks, Berrys, pending ship, slot, and dead NPCs', () => {
  ...
  it('migrates v7 and rejects v6 and invalid non-active slots', () => {
    ...
    expect(migrated).toMatchObject({ version: 9, ship: { shipId: 'starter_sloop', health: 20 }, isLeader: true, passengerNpcIds: [], berries: 0 });
    ...
    expect(deserializeGameState(JSON.stringify(v8))).toMatchObject({ version: 9, isLeader: true, passengerNpcIds: [] });
