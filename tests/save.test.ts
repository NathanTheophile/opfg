...
describe('save v12', () => {
  ...
  it('migrates v7 and rejects v6 and invalid non-active slots', () => {
    ...
    expect(migrated).toMatchObject({ version: 12, ship: { shipId: 'starter_sloop', health: 20 }, isLeader: true, passengerNpcIds: [], berries: 0 });
    ...
    expect(deserializeGameState(JSON.stringify(v8))).toMatchObject({ version: 12, isLeader: true, passengerNpcIds: [] });
    ...
    expect(deserializeGameState(JSON.stringify(v9))).toMatchObject({ version: 12, player: { stats: { agility: 25 }, profile: { familyStructureId: null, socialClassId: null } } });
    ...
    expect(deserializeGameState(JSON.stringify(v10))).toMatchObject({ version: 12, player: { profile: { familyStructureId: null, socialClassId: null } } });
