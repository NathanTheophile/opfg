...
describe('GameState v22', () => {
  it('creates an isolated JSON-compatible origin state', () => {
    ...
    expect(first).toMatchObject({ version: 22, careerPhase: 'origins', ageMonths: 0, slotInMonth: 0, player: { career: { affiliationId: 'civilian', reputation: 0, bounty: 0, rankId: null, titleId: null } }, ship: null, pendingShip: null, maritimeEmergency: null, isLeader: true, passengerNpcIds: [], berries: 0, endingId: null });
    expect(first.player.stats.agility).toBe(25);
    expect(first.player.inventory).toEqual({ capacity: 2, stacks: [] });
    expect(first.player.companion).toBeNull();
