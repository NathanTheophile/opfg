...
> **ACTIVE_V1_LOCK — 2026-08-16.** The Active redesign is now locked by `content/events/v2/ACTIVE_V1_BATCH_CONTRACT.md`. It supersedes older Active passages where they conflict. Active uses **1 root/month**, a **40-year / 480 ageMonths safety horizon**, and four V1 careers only: `civilian`, `pirate`, `marine`, `revolutionary`. `bounty_hunter` and authored career changes are out of V1 scope. `royal_family` remains valid preserved Childhood content but is temporarily **locked for new Origins selection** until a dedicated Royal Childhood → Active transition exists; existing Royal saves/states remain supported. Civilian has no profession subsystem; its V1 Personal Affiliation line is commercial. During the first three Active years (`180 <= ageMonths < 216`), Civilian/Pirate/Revolutionary content must strongly surface crewmate recruitment opportunities, and a recruitment-focused Event must be ineligible when its fixed target CrewRole is already occupied; Marine is exempt and may initially rely on superiors/institutional transport. Ordinary Blue travel is sea-first and destination-random rather than direct destination choice; Reverse Mountain requires the annual crew Navigator power or authored Marine transport. Navigator direct destination choice is disabled in Paradise and may target any runtime island worldwide from the New World. Standard departure requires two local roots after arrival. The first complete Active corridor must end with the real six-axis `/100` score.
