...
> **ACTIVE_V1_LOCK — 2026-08-16.** The Active redesign is now locked by `content/events/v2/ACTIVE_V1_BATCH_CONTRACT.md`. It supersedes older Active passages where they conflict. Active uses **1 root/month**, a **35-year / 420 ageMonths V1 horizon Ending**, and four V1 careers only: `civilian`, `pirate`, `marine`, `revolutionary`. `bounty_hunter` and authored career changes are out of V1 scope. `royal_family` remains valid preserved Childhood content but is temporarily **locked for new Origins selection** until a dedicated Royal Childhood → Active transition exists; existing Royal saves/states remain supported. Civilian has no profession subsystem; its V1 Personal Affiliation line is commercial. During the first three Active years (`180 <= ageMonths < 216`), Civilian/Pirate/Revolutionary content must strongly surface character-first crewmate recruitment opportunities that hand off to runtime Crew-role assignment; Marine is exempt and may initially rely on superiors/institutional transport. Ordinary Blue travel is sea-first and destination-random rather than direct destination choice; Reverse Mountain requires the annual crew Navigator power or authored Marine transport. Navigator direct destination choice is disabled in Paradise and may target any runtime island worldwide from the New World. Standard departure requires two local roots after arrival. The first complete Active corridor must end with the real six-axis `/100` score.
...
>
> **D2.12 Crew runtime authority.** Schema 16 / Save 23 moves Crew-role ownership from `NpcDefinition` to runtime `NpcState.crewRoleId`. NPC identity and CrewRole are separate: any recruited NPC may receive any currently assignable role through Crew management, with no stat prerequisite or NPC-specific role list. One crewmate holds exactly one role, and one role has at most one holder. The current roster is `navigator`, `medic`, `shipwright`, `recruiter`, `first_mate`, `helmsman`, `cook`, `musician`, `scholar`, `foreman`; `gunner`, `fighter`, and `quartermaster` are removed V1 role IDs. Recruitment content tests candidate/story/capacity eligibility and then hands off to role assignment; it must not gate the candidate by target-role vacancy. Active role powers require a ship. Full role reassignment is available only at biological year boundaries; a vacated role stays unavailable until the next yearly reassignment.
...
Un NPC crew possède au maximum un rôle runtime stocké dans `NpcState.crewRoleId`. Ce rôle ne fait pas partie de son identité authorée : un médecin de fiction peut devenir `navigator`, et un personnage récurrent peut être recruté sans rôle mécanique prédéfini. Un crewmate possède exactement un rôle après assignation, et un rôle ne peut avoir qu’un seul holder.

Une Condition telle que `hasCrewRole(navigator)` teste l’assignation runtime actuelle, afin d’ouvrir ou griser un Choice ou d’activer un modificateur explicitement authoré. Un acteur Dice `crewRole` résout également le holder actuel du rôle. Les rôles passifs et les Companions actifs produisent des modificateurs effectifs dérivés pour le joueur et le crew ; le contenu ne doit pas les réappliquer manuellement.

Les rôles actifs (`navigator`, `medic`, `shipwright`, `recruiter`, `first_mate`) ont chacun un pouvoir annuel de rôle, utilisé comme action système et non comme Choice d’Event. Leur cooldown appartient au rôle, pas au NPC. Leur usage requiert un bateau. Le `recruiter` demande au runtime une opportunité de recrutement normale actuellement éligible. Le `first_mate` peut recharger un autre rôle actif déjà utilisé dans l’année, jamais lui-même.

La réassignation complète des rôles n’est disponible qu’à la frontière d’une année biologique. Hors nouvelle recrue sans rôle et réassignation annuelle, il n’y a pas de réassignation normale en milieu d’année. Un rôle libéré par mort, départ ou indisponibilité reste indisponible jusqu’à la prochaine réassignation annuelle.
...
Le contrat de contenu doit supporter au minimum `hasCrew`, `crewSizeAtLeast`, `hasCrewRole` et `isLeader`, ainsi qu’un Effect de changement de leadership. `setNpcStatus` reste l’Effect unique pour recrutement, départ et mort ; l’assignation de rôle est une action système de Crew management, pas un Effect narratif générique.
