...
`CONTENT_SCHEMA_VERSION = 7` versions the authoring/runtime content contract independently from save `GameState.version = 15`. Runtime content validation rejects unsupported schema versions and missing source-French keys. Missing English entries are valid.
