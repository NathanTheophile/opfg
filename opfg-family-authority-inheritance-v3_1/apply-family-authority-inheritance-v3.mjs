...
if (!game.includes("Ces cinq checkpoints sont les couches temporelles d'un **DAG narratif connecté**")) {
  const familyBlock = /Une Childhood complète contient \*\*exactement cinq root Events de Family Saga\*\*[\s\S]*?Chaque chapitre doit posséder exactement un fallback universel,[^\n]*\./;
  if (familyBlock.test(game)) {
    game = game.replace(familyBlock, newGameFamily);
  } else {
    game = game.replace(
      /Chaque chapitre est un pool horizontal de variantes[^\n]*\./,
      "Ces cinq checkpoints forment les couches d'un DAG narratif connecté : après la couche 1, la sélection reste limitée aux descendants du node réellement vécu, puis réagit à l'état actuel et à History."
    );
    game = game.replace(
      /Chaque chapitre doit posséder exactement un fallback universel,[^\n]*\./,
      "La couche 1 conserve un fallback générique ; à partir de la couche 2, les fallbacks sont route-local et couvrent exactement les nodes de la couche précédente."
    );
    console.log("INFO GAME_DESIGN Family section had a different local shape; applied D2.11 override and repaired known stale sentences when found.");
  }
}
...
if (bible.includes(oldBibleFamily)) {
  bible = bible.replace(oldBibleFamily, newBibleFamily);
} else {
  bible = bible.replace(
    "- A chapter is a horizontal variant pool, not a fixed Scheduled chain.",
  ...
  console.log("INFO CONTENT_BIBLE Family section had a different local shape; repaired known D2 statements and will append the D2.11 override.");
}
...
if (bible.includes(oldSpecial)) bible = bible.replace(oldSpecial, newSpecial);
else if (!bible.includes("Royal × Orphan")) {
  bible = bible.replace(
    /The reference V1 example is `Marine × Giant`:[^\n]*/,
    newSpecial
  );
}
...
if (concept.includes(oldMajorSection)) {
  concept = concept.replace(oldMajorSection, newMajorSection);
} else if (!concept.includes("`family_marine` — integrated")) {
  concept = appendOnce(
    concept,
    "<!-- D2.11_ACCEPTED_MAJOR_SAGAS -->",
    `<!-- D2.11_ACCEPTED_MAJOR_SAGAS -->\n${newMajorSection}`
  );
  console.log("INFO EVENT_CONCEPT_INDEX_V2 Major section had a different local shape; appended the accepted D2.11 Saga ledger instead.");
}
...
console.log("D2.11 Family authority + inheritance rework V3.1 applied.");
console.log("");
