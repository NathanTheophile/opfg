...
export class D20Scene {
  ...
  public roll(result: number, rollKey: string | number): void {
    ...
    const bounceCount = 1 + Math.floor(rng() * 3);
    ...
    const flightMs = range(rng, 440, 510);
    const bounceMs = range(rng, 255, 310);
    const settleMs = range(rng, 340, 400);
    ...
    const animation: RollAnimation = {
      ...
      bounceCount,
      bounceHeight: range(rng, 0.31, 0.41),
      impactAt,
      settleAt,
      target,
    };
  ...
  private updatePosition(animation: RollAnimation, p: number): void {
    const { impactAt, settleAt, bounceCount } = animation;
    const side = Math.sign(animation.startPosition.x) || 1;

    if (p < impactAt) {
      const u = p / impactAt;
      const horizontal = easeOutQuad(u);
      const arc = Math.sin(u * Math.PI) * 1.08;

      this.die.position.set(
        THREE.MathUtils.lerp(animation.startPosition.x, side * 0.72, horizontal),
        THREE.MathUtils.lerp(animation.startPosition.y, -0.055, u) + arc,
        THREE.MathUtils.lerp(animation.startPosition.z, side * 0.09, horizontal),
      );

      const scale = THREE.MathUtils.lerp(0.88, 1, easeOutCubic(u));
      this.die.scale.setScalar(scale);
      return;
    }

    if (p < settleAt) {
      const bounceProgress = (p - impactAt) / (settleAt - impactAt);
      ...
      const energy = 0.64 ** bounceIndex;
      const height = animation.bounceHeight * energy;
      const bounce = Math.sin(local * Math.PI) * height;

      // Each rebound travels to a distinct contact point. The first one carries
      // most of the horizontal momentum, then the distance quickly damps out.
      const segmentStart = bounceIndex / bounceCount;
      const segmentEnd = (bounceIndex + 1) / bounceCount;
      const startTravel = easeOutCubic(segmentStart);
      const endTravel = easeOutCubic(segmentEnd);
      const contactTravel = THREE.MathUtils.lerp(startTravel, endTravel, easeInOutQuad(local));
      const lateralArc = Math.sin(local * Math.PI) * side * 0.07 * energy;
      const depthArc = Math.sin(local * Math.PI) * 0.055 * energy * (bounceIndex % 2 === 0 ? 1 : -1);

      this.die.position.set(
        THREE.MathUtils.lerp(side * 0.72, 0, contactTravel) + lateralArc,
        -0.055 + bounce,
        THREE.MathUtils.lerp(side * 0.09, 0, contactTravel) + depthArc,
      );

      // Small squash/stretch makes contacts readable without turning the die into jelly.
      const contact = Math.exp(-local * 18) * energy;
      const apex = Math.sin(local * Math.PI) * energy;
      this.die.scale.set(
        1 + contact * 0.045 - apex * 0.008,
        1 - contact * 0.07 + apex * 0.018,
        1 + contact * 0.045 - apex * 0.008,
      );
      return;
    }

    const u = (p - settleAt) / (1 - settleAt);
    const settleBob = Math.sin(u * Math.PI) * 0.018 * (1 - u);
    this.die.position.set(0, FINAL_POSITION.y + settleBob, 0);

    const overshoot = Math.sin(u * Math.PI) * 0.012 * (1 - u);
    this.die.scale.set(1 - overshoot, 1 + overshoot * 1.35, 1 - overshoot);
  }
