...
export type FinalScoreTier = 'ordinary' | 'notable' | 'remarkable' | 'legendary' | 'mythic';
...
export function buildFinalRunReport(
  ...
  const score = Math.min(100, axes.reduce((total, axis) => total + axis.points, 0));
  ...
  const knownNpcs = Object.entries(state.npcs).filter(([, npc]) => npc.status !== 'unavailable');
  const closestNpc = knownNpcs
    .filter(([, npc]) => npc.status !== 'dead')
    .sort((a, b) => b[1].relationship - a[1].relationship || a[0].localeCompare(b[0]))[0] ?? null;
  const strongestAttributeId = PLAYER_ATTRIBUTES
    .slice()
    .sort((a, b) => state.player.stats[b] - state.player.stats[a] || a.localeCompare(b))[0];
  ...
  return {
    ...
    facts: {
      ...
      totalHakiLevels: Object.values(state.player.powers.haki).reduce((total, level) => total + level, 0),
    },
  };
}
...
function calculateScoreAxes(
  ...
  const attributePower = PLAYER_ATTRIBUTES.reduce((sum, id) => sum + state.player.stats[id], 0)
    / (PLAYER_ATTRIBUTES.length * 50);
  const hakiPower = Object.values(state.player.powers.haki).reduce((sum, level) => sum + level, 0) / 15;
  const fruitPower = state.player.powers.devilFruitId === null
    ? 0
    : 0.35 + 0.65 * (state.player.powers.devilFruitAwakening / 10);
  const power = clamp01(attributePower * 0.7 + Math.max(hakiPower, fruitPower) * 0.3);

  const crew = Object.values(state.npcs).filter(({ status }) => status === 'crew');
  const crewBreadth = clamp01(crew.length / Math.max(1, catalog.crewRoles.length));
  const positiveRelationships = Object.values(state.npcs)
    .filter(({ status, relationship }) => status !== 'unavailable' && status !== 'dead' && relationship > 0)
    .map(({ relationship }) => relationship / 100);
  const relationshipStrength = positiveRelationships.length === 0
    ? 0
    : positiveRelationships.reduce((sum, value) => sum + value, 0) / positiveRelationships.length;
  const relationships = clamp01(crewBreadth * 0.6 + relationshipStrength * 0.4);

  const shipValue = state.ship
    ? catalog.ships.find(({ id }) => id === state.ship!.shipId)?.priceBerries ?? 0
    : 0;
  const marketValue = [...state.player.inventory.stacks, ...(state.ship?.cargo ?? [])]
    .reduce((total, stack) => {
      const item = catalog.items.find(({ id }) => id === stack.itemId);
      return total + (item?.market?.basePriceBerries ?? 0) * stack.quantity;
    }, 0);
  const assetAnchor = Math.max(1, ...catalog.ships.map(({ priceBerries }) => priceBerries));
  const assets = clamp01(1 - Math.exp(-(state.berries + shipValue + marketValue) / assetAnchor));
  ...
  const legacy = clamp01((state.endingId === null ? 0 : 0.5) + Math.min(5, majorRoots) / 10);
  ...
  return (Object.keys(SCORE_WEIGHTS) as FinalScoreAxisId[]).map((id) => ({
    id,
...
function careerAccomplishment01(
  ...
  const rankProgress = rankIndex < 0 || ladder.length === 0 ? 0 : (rankIndex + 1) / ladder.length;
  const titleProgress = career.titleId === 'legend'
    ...
    : career.titleId === 'veteran'
      ...
      : career.titleId === 'rookie'
        ? 0.33
        : 0;
  const bountyProgress = maxBounty <= 0 ? 0 : clamp01(Math.log10(maxBounty + 1) / 8);

  return clamp01(Math.max(rankProgress, titleProgress, bountyProgress));
}

function deriveMaxBounty(state: GameState, catalog: ContentCatalog): number {
  let bounty = 0;
  ...
  for (const entry of state.history) {
    ...
    for (const effect of outcome.effects) {
      if (effect.type === 'setBounty') bounty = Math.max(0, effect.value);
      if (effect.type === 'modifyBounty') bounty = Math.max(0, bounty + effect.amount);
...
function deriveHighestRank(state: GameState, catalog: ContentCatalog): CareerRankId | null {
  let best: { id: CareerRankId; sortOrder: number } | null = null;
  const consider = (rankId: CareerRankId | null) => {
    if (rankId === null) return;
    const rank = catalog.careerRanks.find(({ id }) => id === rankId);
    if (rank && (best === null || rank.sortOrder > best.sortOrder)) {
      best = { id: rank.id, sortOrder: rank.sortOrder };
    }
  };

  consider(state.player.career.rankId);
  for (const entry of state.history) {
    const outcome = outcomeForEntry(entry, catalog);
    outcome?.effects.forEach((effect) => {
      if (effect.type === 'setCareerRank') consider(effect.rankId);
    });
  }
  return best?.id ?? null;
}

function selectLifeMoments(history: HistoryEntry[], catalog: ContentCatalog): FinalRunMoment[] {
  ...
  history.forEach((entry, index) => {
    const event = catalog.events.find((candidate) => candidate.id === entry.eventId);
    if (event?.kind === 'normal' && event.majorTrack !== undefined) selected.add(index);
  });

  for (let slot = 1; selected.size < targetCount && slot < targetCount - 1; slot += 1) {
    selected.add(Math.round((slot * (history.length - 1)) / (targetCount - 1)));
  }
  for (let index = 0; selected.size < targetCount && index < history.length; index += 1) selected.add(index);
...
function outcomeForEntry(entry: HistoryEntry, catalog: ContentCatalog): Outcome | null {
  const event = catalog.events.find((candidate) => candidate.id === entry.eventId);
  if (!event) return null;
  const choice = event.choices.find(({ id }) => id === entry.choiceId);
  if (!choice) return null;
  if (choice.resolution.type === 'deterministic') {
    return choice.resolution.outcome.id === entry.outcomeId ? choice.resolution.outcome : null;
  }
  return Object.values(choice.resolution.outcomes).find(({ id }) => id === entry.outcomeId) ?? null;
}
