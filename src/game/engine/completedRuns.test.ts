...
import { archiveCompletedRun, COMPLETED_RUNS_KEY, loadCompletedRuns } from './completedRuns';
...
describe('completed run history', () => {
  it('archives ended runs once and keeps active saves out', () => {
    ...
    const ended = { ...active, careerStatus: 'ended' as const, careerEndReason: 'legacy' as const, endingId: 'career_complete' };
    archiveCompletedRun(storage, ended, 10);
    archiveCompletedRun(storage, ended, 20);
