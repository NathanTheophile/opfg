...
export function selectNextEvent(state: GameState, catalog: ContentCatalog): GameState {
  ...
  const critical = selectCriticalEvent(state, catalog.events);
...
function selectCriticalEvent(state: GameState, events: readonly EventDefinition[]): EventDefinition | undefined {
