...
export function loadCompletedRuns(storage: StorageLike): CompletedRun[] {
  try {
    ...
    return parsed.flatMap((value): CompletedRun[] => {
      if (!isRecord(value) || typeof value.id !== 'string' || typeof value.completedAt !== 'number' || !Number.isFinite(value.completedAt)) return [];
      const state = deserializeGameState(JSON.stringify(value.state));
      return state?.careerStatus === 'ended'
        ? [{ id: value.id, completedAt: value.completedAt, state }]
        : [];
...
export function archiveCompletedRun(
  ...
  const next = [{ id, completedAt, state: structuredClone(state) }, ...existing]
    .sort((a, b) => b.completedAt - a.completedAt || a.id.localeCompare(b.id));
  try {
